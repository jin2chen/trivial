...
class yiiDebugDB extends yiiDebugClass
{
	public static function getInfo($data, $config = null)
	{
		...
		foreach ($data as $row)
		{
			...
			{
				$exclude1 = 'Querying SQL: SHOW CREATE TABLE';
				$exclude2 = 'Querying SQL: SHOW COLUMNS FROM';
				$len1 = strlen($exclude1);
				$len2 = strlen($exclude2);
				if ($row[2] == 'system.db.CDbCommand' 
				&& strncmp($row[0], $exclude1, $len1) && strncmp($row[0], $exclude2, $len2)) 
				{
					$items[] = $row;
					$count++;
				} elseif ($row[2] != 'system.db.CDbCommand') {
					$items[] = $row;
				}
			}
		}
...
class XWebDebugRouter extends CLogRoute
{
	...
	public function processLogs($logs)
	{
		...
		if (!defined('YII_DEBUG_TB')) return;
